...
import calendar
# 已移除废弃/未使用 import: cPickle as pickle
# 已移除废弃/未使用 import: from six import StringIO
...
def get_period_date(peroid, start_date, end_date):
    ...
    period_stock_data = stock_data.resample(peroid).last()  # 替换 resample(..., how='last') → .resample(...).last()
...
def GetPredict(context, weight):
    ...
    df_data = df_data.sort_values(ascending=False)  # 替换 sort(ascending=False) → sort_values(ascending=False)
    return df_data