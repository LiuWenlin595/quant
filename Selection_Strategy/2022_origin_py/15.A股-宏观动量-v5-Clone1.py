...
# 已移除废弃/未使用 import: cPickle, seaborn, matplotlib, os, statsmodels, scipy, talib
# 注：聚宽平台不支持本地路径读取，需替换为聚宽数据源或上传数据文件
...
# path = '/home/jquser/Macro'  # 聚宽平台无法访问本地路径，需注释或替换
...
class backtest_result():
    ...
    def Max_Drawback(self):
        ...
        for j in range(0, len(net_value), 1):
            max_value = max(max_value, df_tmp.iloc[j, 'value'])
            df_tmp.iloc[j, 'drawback'] = 1 - df_tmp.iloc[j, 'value'] / max_value
    ...
    def Sharpe(self):
        ...
        annual_pct = df_tmp.iloc[-1, 'value'] ** (12. / len(df_tmp)) - 1
...
# PMI择时
# body=read_file(path+'/PMI组合.xls')  # 聚宽平台不支持本地文件读取
...
prices_M = prices.resample('M').last()  # 替换弃用的 how='last'
rate_riskfree = 0
df_pct = pd.DataFrame()
