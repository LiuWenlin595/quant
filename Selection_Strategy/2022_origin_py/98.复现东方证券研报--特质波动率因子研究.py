...
def before_market_open(context):
    ...
    #排序
    factor = factor.sort_values('score', ascending = g.asc)
...
def set_slip_fee(context):
    ...
    if dt>dt.datetime(2013,1, 1):
        set_commission(PerTrade(buy_cost=0.0003, 
                                sell_cost=0.0013, 
                                min_cost=5)) 
        
    elif dt>dt.datetime(2011,1, 1):
        set_commission(PerTrade(buy_cost=0.001, 
                                sell_cost=0.002, 
                                min_cost=5))
            
    elif dt>dt.datetime(2009,1, 1):
...
def FF3(stocks, end_date, rf = 0.04):
    ...
    #选出特征股票组合
    S=df.sort_values('circulating_market_cap').index.tolist()[:LoS//3]
    B=df.sort_values('circulating_market_cap').index.tolist()[LoS-LoS//3:]
    L=df.sort_values('BP').index.tolist()[:LoS//3]
    H=df.sort_values('BP').index.tolist()[LoS-LoS//3:]
...
def linreg(X,Y):
    ...
    return results.resid.std() * np.sqrt(252)
...
def CARHART(stocks, end_date, rf = 0.04):
    ...
    LO = df42.sort_values('ret').index.tolist()[:LoS//3]
    W = df42.sort_values('ret').index.tolist()[LoS-LoS//3:]
...
def FF5(stocks, end_date, rf = 0.04):
    ...
    ldf = get_fundamentals(q, start_date - dt.timedelta(365))
    ...
    L=df.sort_values('BP').index.tolist()[:LoS//3]
    H=df.sort_values('BP').index.tolist()[LoS-LoS//3:]
    W=df.sort_values('roe').index.tolist()[:LoS//3]
    R=df.sort_values('roe').index.tolist()[LoS-LoS//3:]
    C=df.sort_values('Inv').index.tolist()[:LoS//3]
    A=df.sort_values('Inv').index.tolist()[LoS-LoS//3:]
