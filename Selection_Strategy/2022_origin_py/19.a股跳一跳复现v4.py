...
open_df = data.loc['open'].iloc[1:]
close_df = data.loc['close']
close_p = np.array(data.loc['close'].iloc[0:-1])
open_p = np.array(data.loc['open'].iloc[1:])
jump = pd.DataFrame(np.multiply(open_p, 1/close_p) - 1, columns=open_df.columns, index=open_df.index)
#10日滑动平均
...
def get_alpharank(alpha, date):
    ...
    t = pd.DataFrame(np.argsort(np.array(alpha0)), index=alpha0.index)
    return t  
...
#获取各组收益率统计
group, net_value, return_m = get_group(close_df, alpharank, alpharank.index, 10)
fig = plt.figure()
...
jump_alpha = pd.DataFrame(abs(np.multiply(open_p, 1/close_p) - 1), columns=open_df.columns, index=open_df.index)
...
#获取各组收益率统计
group, net_value, return_m = get_group(close_df, alpharank, alpharank.index, 5)
fig = plt.figure()
...
#求动量因子
close_monthly = close_df.resample('M').last()
...
new_alpha = pd.DataFrame(abs(np.multiply(open_p, 1/close_p) - alpha_20 - 1), columns=open_df.columns, index=open_df.index)
new_alpha_monthly = new_alpha.resample('M').last()
new_alpharank = get_alpharank(new_alpha_monthly, new_alpha_monthly.index)
#测试因子变后的效果
group, net_value, return_m = get_group(close_df, new_alpharank, new_alpharank.index, 5)
plt.figure()
fig = plt.figure(figsize=(13, 5))
ax1 = fig.add_subplot(1, 1, 1)
for i in range(5):
...
for i in range(len(close_df.columns)):
    array = np.array(close_df.iloc[:,i])
    p = array[-1]
