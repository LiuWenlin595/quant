...
# 导入函数库
# 已移除废弃/未使用 import: from pandas.stats.api import ols
...
## 开盘时运行函数 —— 兼容性修复：替换 handle_data 中的 .mavg() 为 history + 自算均线
def handle_data(context, data):
    record(cash=context.portfolio.cash)
    
    # 获取上证指数历史收盘价
    hist = history(60, unit='1d', field='close', security_list=['000001.XSHG'], df=True)
    ma1 = hist['000001.XSHG'].iloc[-1]  # 最近1日即当日收盘价
    ma5 = hist['000001.XSHG'].iloc[-5:].mean()
    ma10 = hist['000001.XSHG'].iloc[-10:].mean()
    ma20 = hist['000001.XSHG'].iloc[-20:].mean()
    ma30 = hist['000001.XSHG'].iloc[-30:].mean()
    ma60 = hist['000001.XSHG'].iloc[-60:].mean()
...
def trade_func(context):
    ...
    #取排名top N的股票 进行轮动 —— 兼容性修复：sort → sort_values
    df = df.sort_values('peg', ascending=True)[:g.stock_num]
