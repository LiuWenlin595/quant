...
def train_pred(factor, hs, period, alg):
    ...
    for i in np.arange(len(factor) - period):
        x_train, y_train = factor[i:i+period], hs.iloc[i:i+period]['close']
        x_test = np.array(factor[i+period])
        y_test = hs['close'].iloc[i+period]
        
        alg.fit(x_train, y_train)
    
        temp = alg.predict(x_test.reshape(1, -1))
        pred.append(temp[0])
        ytest.append(y_test)
    return pred, ytest