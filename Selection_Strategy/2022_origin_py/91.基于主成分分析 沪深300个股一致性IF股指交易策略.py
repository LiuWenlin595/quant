...
def change_old_ins(context, ins):
    ...
    if now_dom not in short_los.keys():
        if len(short_los.keys()) >= 1:
            Lasthold = list(short_los.keys())[0]
            ...
            g.short_tradebook[context.current_dt] = Trade
    g.dom = now_dom
...
def cal_ATR():
    ...
    high = np.array(index_price['high'])
    low = np.array(index_price['low'])
    close = np.array(index_price['close'])
...
def loststop(context):
    ...
    near_close = np.array(near_close_array[-1])
    if len(g.long_tradebook) > 0:
        for date in list(g.long_tradebook.keys()):
    ...
    if len(g.short_tradebook) > 0:
        for date in list(g.short_tradebook.keys()):
...
def move_average_uni():
    if len(g.std_uni_list) >= g.move_average_term:
        arr_uni_list = np.array(g.std_uni_list)
