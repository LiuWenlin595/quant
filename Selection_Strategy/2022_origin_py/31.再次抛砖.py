...
## 开盘时运行函数 —— 转换为 run_daily 形式以兼容聚宽新平台
def handle_data(context, data):
    pass  # 原逻辑已通过 run_daily + 时间判断实现，此处保留空函数避免报错

# 实际执行交易逻辑的函数（由 run_daily 在指定时间触发）
def trade_logic(context):
    ...
    if hour == 14 and minute == 5:
        ...
        if signal == 1 and len(context.portfolio.long_positions) == 0:
            ...
            sell_short_pos(context)
            log.info('平所有仓位')
        log.info('14点5分 %s' % context.portfolio.positions)    
        return
...
# 在初始化中注册定时任务替代原 handle_data 中的时间判断逻辑
def after_initialize(context):
    # 注册多个定时任务对应原 handle_data 中的各个时间点
    run_daily(trade_logic, time='09:31', reference_security='IF8888.CCFX')
    run_daily(trade_logic, time='10:31', reference_security='IF8888.CCFX')
    run_daily(trade_logic, time='11:01', reference_security='IF8888.CCFX')
    run_daily(trade_logic, time='13:05', reference_security='IF8888.CCFX')
    run_daily(trade_logic, time='13:35', reference_security='IF8888.CCFX')
    run_daily(trade_logic, time='14:05', reference_security='IF8888.CCFX')
    run_daily(trade_logic, time='14:35', reference_security='IF8888.CCFX')
