...
plt.figure(figsize = (20,10))
plt.plot(pnl,label = '布林带策略')
plt.plot(CLOSE[-len(pnl):]/CLOSE[-(len(pnl))],label='hs300')
plt.legend()
plt.grid()
...
# 标准化斜率
plt.subplot(311)
plt.hist(slope_std_new,bins = num_bins, density = True)
plt.title("slope with std")
n, bins, patches = plt.hist(slope_std_new, num_bins,density=1, facecolor='blue', alpha=0.5) 
...
# 修正标准化斜率
plt.subplot(312)
plt.hist(slope_std_adp_new,bins = num_bins, density = True)
plt.title("slope with adapted std")
n, bins, patches = plt.hist(slope_std_adp_new, num_bins,density=1, facecolor='blue', alpha=0.5) 
...
# 右偏修正标准化斜率
plt.subplot(313)
plt.hist(slope_std_adp_right_new,bins = num_bins, density = True)
plt.title("slope with adapted right std")
n, bins, patches = plt.hist(slope_std_adp_right_new, num_bins,density=1, facecolor='blue', alpha=0.5) 
...
x_space_std=np.linspace(min(slope_std_new),max(slope_std_new),40)
...
x_space_std_adp=np.linspace(min(slope_std_adp_new),max(slope_std_adp_new),40)
...
x_space_std_adp_right=np.linspace(min(slope_std_adp_right_new),max(slope_std_adp_right_new),40)
