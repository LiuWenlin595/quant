...
def get_buy_list(context):
    ...
    for date in past_days_list:
        ...
        price1 = np.array(get_price(g.stock_pool, time1, time1, '1m', ['close'])['close'])
        price2 = np.array(get_price(g.stock_pool, time2, time2, '1m', ['close'])['close'])
    ...
    df_rank_index = df_rank_index.sort_values(by='rate')  # 修复：sort → sort_values
    df_rank_index = df_rank_index.index
    buy_list = df_rank_index[int((g.group-1)*0.1*len(g.stock_pool)*0.25):int(g.group*0.1*len(g.stock_pool)*0.25)]  # 修复：切片索引转为整数
    return buy_list


# 未定义函数占位（原策略缺失，保留调用关系）
def revers_factor_get(context):
    return get_buy_list(context)