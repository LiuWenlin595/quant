...
# 将原 handle_data 逻辑迁移到每日定时执行函数中
def check_exit_condition(context):
    if g.day_count <= 1:
        return
        
    position = context.portfolio.positions[g.open_security]
    # 使用 history 获取最新价格替代 data[security].price
    current_price = history(1, '1d', 'close', position.security, df=False)[0]
...
# 替换原 handle_data，在每天14:59执行检查逻辑
run_daily(check_exit_condition, time='14:59', reference_security='000300.XSHG')