...
#  开始处理每一个标的买入，卖出操作
    for stock_name in security[:]:
       # 获取该股票的PE,PB
        ...
        m_pe = df['pe_ratio'].values[0] if len(df['pe_ratio'].values) > 0 else None
        m_pb = df['pb_ratio'].values[0] if len(df['pb_ratio'].values) > 0 else None
        m_roe = df['roe'].values[0] if len(df['roe'].values) > 0 else None
        ...
        if curr_value == 0 and curr_price < curr_buy and m_pe is not None and m_pe < 7 and m_pb is not None and m_pb < 1 and ma_flag == -1 : 
            if cash_flag == 1  :
...
# 金字塔补仓： 进行金字塔式补仓，降低仓位成本。条件是：买入线以下，当前价比成本价低10%，均线拐头，仓位没到最大仓位。
            if bc_flag == 1 and curr_price < curr_buy and curr_price <  curr_avg_cost * 0.95 and m_pe is not None and m_pe < 7 and m_pb is not None and m_pb < 1 and ma_flag == -1 and curr_value < m_max :
...
# 定投补仓： 如亏损状态，进行定期定额定投一次，降低仓位成本。条件是：买入线以下，当前价比成本价低，仓位没到最大仓位，每20个交易日定投一次。
            elif dt_flag == 1 and curr_price < curr_buy and curr_price < curr_avg_cost * 0.99 and m_pe is not None and m_pe < 7 and m_pb is not None and m_pb < 1 and g.days[stock_name] % 10 == 0 and curr_value < m_max :    # 每10个交易日定投一次
                g.last_price[stock_name]    = curr_price                                                 #  成交价设定为上次交易价
                if cash_flag == 1  :
...
# 网格买入1： 在低成本区域,如未满仓,采用网格交易，降低仓位成本。条件是：买入线以下，当前价比上次交易价低5%，仓位没到最大仓位1.2倍。
            elif wg_flag == 1 and curr_value < m_max * 1.0 and curr_price < curr_sell and m_pe is not None and m_pe < 7 and m_pb is not None and m_pb < 1  and curr_price < g.last_price_wg[stock_name] * 0.95 :
...
#            elif curr_price > curr_sell and (curr_price > curr_avg_cost * 5  or m_pe < 12 or m_pb > 1.5 or (curr_price > curr_avg_cost * 3 and  ma_flag == 1)) : 
            elif curr_price > curr_sell and (curr_price > curr_avg_cost * 5   or (m_pe is not None and m_pe > 10) or (m_pb is not None and m_pb > 1.5)  or (curr_price > curr_avg_cost * 3 and  ma_flag == 1)) : 
                g.total_count[stock_name] = 0     #  买入次数清0
                g.days[stock_name]        = 0     #  初始化持仓天数0
                order_target(stock_name,0)        #  股票翻倍时卖出所有股票
