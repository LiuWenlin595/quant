...
def set_buyableList(context):
    ...
    money4CheckSeries.sort_values(ascending = False, inplace=True)
    ...
    ret4CheckSeries.sort_values(ascending = False, inplace=True)
...
def before_trading_start(context):
    ...
    #现持仓不为空且现持仓就是今能买
    elif list(context.portfolio.positions.keys()) == g.buyableList:
        security = g.buyableList[0]
        #判断现持仓需要卖吗
        if buyOrSellCheck(security, context) == -1:
        ...
        security2 = list(context.portfolio.positions.keys())[0]
    ...
    if g.sellDict != {}:
        message = '卖 ' + list(g.sellDict.keys())[0]
        print(message)
        send_message(message, channel = 'weixin')
    if g.buyDict != {}:
        message = '买 ' + list(g.buyDict.keys())[0]    
        print(message)
        send_message(message, channel = 'weixin')
