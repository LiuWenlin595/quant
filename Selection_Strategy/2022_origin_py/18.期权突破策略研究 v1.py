...
#     首先，我们进行期权交易的可行性分析。由于50ETF价格每日变化，每日的期权合约也有可能变化，所以我们首先将每日的期权合约代码取下来，存放在三个列表中，分别是：平值期权合约列表（Code_atm）,实值期权合约列表（Code_itm），虚值期权合约列表（Code_otm)。每个列表包含两列，第一列是看涨期权代码，第二列是看跌期权代码。实值期权与虚值期权分别只取除平值外实一档或者虚一档的期权。
...
#     然后我们对这六种期权的日内收益进行分析，下图是这六种期权的日内收益直方图和统计数据。从图上和统计结果来看，很明显地峰度大于3，期权的日内收益呈厚尾分布。偏度大于0，则说明分布右偏。
...
#     接下来我们定义期权的日内收益与标的的日内收益之比作为有效杠杆，用以统计期权与标的日内涨跌幅的变动关系。$$有效杠杆=\frac{\Delta P/P}{\Delta S/S}$$  
#     从如下的统计结果来看，对于认购的实值、平值、虚值期权它们的均值都大于0，再次说明认购期权日内价格与标的价格变化呈正相关关系；同理，认沽的实值、平值、虚值期权它们的均值都小于0，说明认沽期权日内价格与标的价格变化呈负相关关系。并且对于认沽和认购期权的有效杠杆统计的均值绝对值都远大于1，说明期权交易具有高杠杆性，是一种非常适合投机交易的资产。  
#     综上所述，期权价格变化与标的变化具有明显的正/负相关关系，日内收益具有明显的厚尾现象，且具有高杠杆，是一种非常适合进行投机交易的品种。
...
#     我们按上面提到的策略步骤编写回测代码。在回测过程中有一些因素影响最终的收益效果，以下我们分别对这些因素进行分析，选出最优的因素进行组合，然后在样本外进行测试。这里取样本内时间为：2017-1-1到2018-5-20。需要考虑的因素包括： 
#   
...
#   
#     以下是回测代码框架，初始资金为10000元。首先默认状态设置为取当月实值期权合约，向上突破做多认购期权，向下突破做空认购期权。Dual Thrust默认参数设置为Num=3,k1=0.3,k2=0.3，手续费设置为0.0005。基于需要调整的因素改动相应代码即可，以下为各因素分析结果。
...
#     保持其他参数不变，只更改合约类型，然后运行回测代码比较结果。从图上可以看出，实值期权效果最好，平值次之，虚值最差。这是因为虚值期权价格有时非常便宜，考虑手续费后，手续费占比很高，因此亏损非常厉害。所以不建议选择价格过于便宜的期权。
...
#     从上面的讨论可知，向上突破可以买入认购也可以卖出认沽，向下突破可以卖出认购或买入认沽。保持其他参数不变，变换买入卖出方式，比较性能。有四类买入卖出方式：  
# 1、向上突破买入认购，向下突破卖出认购；  
# 2、向上突破卖出认沽，向下突破买入认沽；  
# 3、向上突破买入认购，向下突破买入认沽；  
# 4、向上突破卖出认沽，向下突破卖出认购。   
#     从图上可以看出，向上突破买入认购，向下突破卖出认购净值曲线最高，效果最好；向上突破卖出认沽，向下突破买入认沽净值曲线最低，效果最差。
...
#     Dual Thrust策略中包含三个参数，历史数据天数，区间参数f1,f2。接下来我们更改参数，测试回测效果。  首先是历史天数,我们依次选择历史天数为1、2、3、5、8、10，保持区间参数发，f1=f2=0.3不变。从下面的实际验证来看，历史天数取3天能保证最终的收益和净值曲线的波动不会太大。
...
#     接下来我们分析区间参数f1,f2的值，首先固定历史天数为1天，这里为方便只取f1=f2,分别取值为0.1，0.2，0.3，0.5，回测结果进行比较。如下所示，其中最优的参数是0.3,不过波动比其它参数大些，然后依次是0.5、0.1、0.2。
...
#     综合以上因素来看，我们选择最优的状态，即选择当月实值期权，向上做多认购，向下做空认购，Dual Thrust参数设置为Num=3,k1=k2=0.3,加上单边手续费0.0005，净值曲线如图。  
...
#     样本外表现为上图。从图上看出，样本外表现在大部分时候表现良好。但是有些段回撤非常大，一天内超过15%的亏损，如在2018-7-20，当天选出的期权为“50ETF购2018年7月2.40（10001355.XSHG）”。50ETF当天的开盘价为2.474，收盘价为2.540，涨幅达2.67%。在开盘后第14分钟即9：44时50ETF价格为2.460,突破下轨2.461,认为是下跌趋势，因此卖出认购期权，卖出价格为0.071。但是后续行情中，50ETF基本呈上涨趋势，方向判断错误，因此产生亏损。如下图。
