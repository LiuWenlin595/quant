...
def delete_nan(factor_list,pl_pro,key=1):
    if key==1:
        ...
        for i in range(1,len(factor_list)):
            temp = pl_pro[factor_list[i]].values
...
def get_before_data(context):
    ...
    g.trade_day = list(map(str,tradedays))
...
def market_open(context):
    ...
    if g.if_trade == True:
        ...
        for i in range(0,len(factor_df.columns)):
            factor_df = factor_df.sort_values(by=factor_df.columns[i],ascending=True)
            rank = range(1,len(factor_df.index)+1)
            factor_df[factor_df.columns[i]+'_rank'] = rank
        rank_sum = np.dot(factor_df.iloc[:,len(g.factor):len(factor_df.columns)].values,np.array(g.weight))
        ...
        factor_df = factor_df.sort_values(by='total_score',ascending=False)
