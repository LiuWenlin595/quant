...
def filter_paused_and_st_stock(stock_list):
    ...
    return [stock for stock in stock_list if not current_data[stock].paused 
            and not current_data[stock].is_st and 'ST' not in current_data[stock].name 
            and '*' not in current_data[stock].name and '退' not in current_data[stock].name]
...
def close_position(context, position):
    ...
    order = order_target_value_(context, security, 0)
