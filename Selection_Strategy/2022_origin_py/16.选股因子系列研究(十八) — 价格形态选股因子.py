...
for date in dateList:
    ...
    # 因子计算
    temp1 = np.log(df_data["high"] / df_data["open"]).mean()
    temp2 = np.log(df_data["close"] / df_data["low"]).mean()
    temp3 = np.log(df_data["avg"] / df_data["close"]).mean()
...
# 分层组合回测的具体数据以及多空组合
IndexList = list(range(1,11)) + ["longshort"]
